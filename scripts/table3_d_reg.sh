...
# Dropout 실험 (0.0, 0.2)
for p in 0.0 0.2; do
    python demo_wmt14_pretrained.py --load_dir ${VOCAB_DIR} --checkpoint_dir "checkpoints/table3_d_drop${p}" --drop_prob ${p} --max_steps 100000 --gradient_checkpointing
...
for ls in 0.0 0.2; do
    python demo_wmt14_pretrained.py --load_dir ${VOCAB_DIR} --checkpoint_dir "checkpoints/table3_d_ls${ls}" --label_smoothing ${ls} --max_steps 100000 --gradient_checkpointing

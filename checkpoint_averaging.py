...
def get_last_n_checkpoints(checkpoint_dir, n=5, pattern='model_step_*.pt'):
    """
    Get last N checkpoints from directory
    ...
    if len(checkpoint_files) == 0:
        raise ValueError(f"No checkpoints found in {checkpoint_dir} with pattern {pattern}")
    ...
    print(f"Found {len(checkpoint_files)} total checkpoints")
    print(f"Selected last {len(checkpoint_paths)} checkpoints:")
